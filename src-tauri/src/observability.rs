...
pub fn write_pack_configs(paths: &PackPaths, cfg: &PackConfig) -> Result<(), String> {
    ...
    std::fs::write(paths.config_dir().join("grafana.ini"), grafana_ini(cfg, paths)).map_err(|e| e.to_string())?;
...
fn compute_pack_status(paths: &PackPaths) -> PackVersionStatus {
    ...
    PackVersionStatus { installed: all_ready, updates_available: updates, components }
}
...
fn is_up_to_date(
    ...
    if !versions.get(component).map(|v| v == version).unwrap_or(true) {
        return false;
    }
...
fn replace_binary(src: &Path, dst: &Path) -> Result<(), String> {
    if dst.exists() && std::fs::remove_file(dst).is_err() {
        ...
        std::fs::rename(dst, &aside).map_err(|e| {
            format!("cannot replace {} (a previous instance may be running — restart the app): {e}", dst.display())
        })?;
    }
...
fn install_blocking(app: tauri::AppHandle) -> Result<PackStatus, String> {
    ...
    for (i, (component, version, exe)) in components.iter().enumerate() {
        ...
        if plan[i] {
            ...
            if versions.get(*component).is_none() {
        ...
        if *component == "grafana" {
            ...
            copy_dir_all(home_src, &home_dst).map_err(|e| format!("grafana-home copy failed: {e}"))?;
        }
        ...
        versions.insert((*component).to_string(), (*version).to_string());
        save_versions(&paths, &versions).map_err(|e| format!("recording installed version failed: {e}"))?;
        emit("done");
    }
...
mod tests {
    ...
    fn grafana_ini_is_localhost_embeddable_and_persists_data_outside_home() {
        ...
        assert!(!c.contains("grafana-home"), "data/provisioning must not be under grafana-home");
    }
    ...
    fn pinned_components_are_the_three_services() {
        ...
        assert!(c.iter().any(|(n, _, e)| *n == "influxdb" && *e == "influxd"));
    }
